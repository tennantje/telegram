...
import { SendMarkdownMessageCommand } from "./commands";
...
export class TelegramClient {
  ...
  public async sendMarkdown(command: SendMarkdownMessageCommand): Promise<SendMessageOutput> {
    const startTime = Date.now();

    try {
      this.loggerFunction?.({
        level: "info",
        message: "Sending Telegram markdown message",
        meta: {
          chatId: command.input.chatId,
          textLength: command.input.text.length,
        },
      ...
      this.loggerFunction?.({
        level: "info",
        message: "Telegram markdown message sent successfully",
        meta: { messageId: response.data.result!.messageId, duration },
      });
